...
require 'raw_recognizer'
  ...
  class PocketsphinxServer::Server < Sinatra::Base
    ...
    def do_post()
      ...
      settings.config['handlers'].each do |handler_config|
        className = handler_config['name']
        requireName = handler_config['require']
        require requireName
        puts "Creating handler #{className}"
      ...
      #Start Timer
      Benchmark.bmbm do |r|
      r.report("Request-Read time statistics") do
      ...
      if length > 0       
        ...
        logger.info "Processing time:" + (end_time-start_time).to_s
